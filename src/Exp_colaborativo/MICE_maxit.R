...
# paquetes necesarios para la imputacion
...
metodo_imputacion <- "regresion"

PARAM$experimento <- paste("EC_HT8231",metodo_imputacion,sep="_")
...
#             seed = PARAM$lgb_semilla,method="norm.predict",printFlag = T,m=5,maxit=50,seed=PARAM$lgb_semilla)

options(future.globals.maxSize = 891289600) # 850Mb = 850*1024^2

plan("multicore", workers = 4)

imp.train <-futuremice(
    dataset[foto_mes %in% PARAM$input$training,..campos_buenos],
    method = 'lasso.norm', 
    parallelseed = 123, #PARAM$lgb_semilla
    n.core = 5, #parallelly::availableCores(logical = FALSE)-1
    m= 5, 
    maxit=10
    #plan = 'multiprocess'
    )
...
# imp.test <- mice.mids(imp.train, newdata = dataset[foto_mes %in% PARAM$input$testing,..campos_buenos],maxit=50)

imp.test <- mice.reuse(mids, newdata = dataset[foto_mes %in% PARAM$input$testing,..campos_buenos],maxit=10, seed =123)

# imp.val <- mice.mids(imp.train, newdata = dataset[foto_mes %in% PARAM$input$validation,..campos_buenos],maxit=2)

# imp.val <- mice.mids(imp.train, newdata = dataset[foto_mes %in% PARAM$input$validation,..campos_buenos],maxit=50)

imp.test <- mice.reuse(mids, newdata = dataset[foto_mes %in% PARAM$input$validation,..campos_buenos],maxit=10, seed =123)

dataset[foto_mes %in% PARAM$input$testing,campos_buenos] <- complete(imp.test)
dataset[foto_mes %in% PARAM$input$training,campos_buenos] <- complete(imp.train)
dataset[foto_mes %in% PARAM$input$validation,campos_buenos] <- complete(imp.val)

write.csv.gz(dataset, file = gzfile("~/buckets/b1/exp/EC_HT8231_regresion/dataset_mice_lassonorm.gz.csv"))
