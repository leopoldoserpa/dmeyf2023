...
metodo_imputacion <- "median_corregido"
...
median_training <- lapply(taining_subset, median, na.rm = TRUE)
...
# Reemplazo por la mediana de training, en todos los datasets! Solo con esto andaría. Siguen habiendo problemas de truncamiento
for (col in names(taining_subset)) {
  dataset[is.na(dataset[[col]]), (col) := median_training[[col]]]
}
