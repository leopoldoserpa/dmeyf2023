...
metodo_imputacion <- "media_numeric_corregido"
...
PARAM$finalmodel$optim$num_iterations <- 657
PARAM$finalmodel$optim$learning_rate <- 0.058039743
PARAM$finalmodel$optim$feature_fraction <- 0.58747641
PARAM$finalmodel$optim$min_data_in_leaf <- 15872
PARAM$finalmodel$optim$num_leaves <- 332
...
for (col in names(taining_subset)) {
  dataset[is.na(dataset[[col]]), (col) := means_training[[col]]]
}
